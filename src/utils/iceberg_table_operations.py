...
class IcebergTableOperations:
    ...
    def get_earliest_record(
        ...
        """
        Get the earliest record for each partition in a table.
        ...
        """
        try:
            logger.info(f"Getting earliest record from table {table_name}")
            df = self.spark.table(table_name)
            df = self.get_partitioned_row_number(df, partition_cols, order_cols, order="asc")
            if filter_date_flag:
                df = df.filter(col("load_dttm").between(start_date, end_date))
            df = df.filter(col("row_number") == 1)
        except Exception as e:
            logger.error(f"Error in get_earliest_record: {e}")
            raise e
        return df
